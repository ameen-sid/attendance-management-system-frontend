...
import {
  Calendar, ChevronLeft, ChevronRight, Search, MapPin, Clock, ArrowRight, Loader2
} from 'lucide-react';
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div>
          ...
          <p className="text-gray-500 mt-1">Monitor daily check-ins and working hours.</p>
        </div>
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            ...
            <tbody className="divide-y divide-gray-100">
              {!loading && currentLogs.length === 0 ? (
                <tr>
                  <td colSpan={5} className="p-8 text-center text-gray-500 text-sm">No logs found for this date.</td>
                </tr>
              ) : (
                currentLogs.map((log) => (
                  <tr
                    ...
                    <td colSpan={5} className="p-0">
                      ...
                      >
                        <div className="p-5 flex-[2] flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-semibold text-gray-900 group-hover:text-indigo-600 transition-colors">{log.user}</p>
                        ...
                        <div className="p-5 flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-medium text-gray-700">{log.clockIn}</span>
                          </div>
                        </div>
                        ...
                        <div className="p-5 flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-medium text-gray-700">{log.clockOut}</span>
                          </div>
                        </div>
                        ...
                        <div className="p-5 flex-1 text-right flex items-center justify-end gap-4">
                          {renderStatus(log)}
                          <ChevronRight size={18} className={`text-gray-300 transition-transform duration-200 ${expandedId === log.id ? 'rotate-90 text-indigo-500' : ''}`} />
                        </div>
                      </div>
                          ...
                          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 bg-white/50 rounded-2xl p-6 border border-indigo-100/50">
                            ...
                            <div className="col-span-1 lg:col-span-2 space-y-6">
                              <div className="flex items-center gap-2 pb-3 border-b border-indigo-100">
                                ...
                                <h4 className="text-xs font-bold text-indigo-600 uppercase tracking-widest">Task Checklist & Report</h4>
                              </div>
                              
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {log.attendanceTasks && log.attendanceTasks.length > 0 ? (
                                  log.attendanceTasks.map((task: any) => (
                                    <div key={task.id} className="bg-gray-50/50 p-4 rounded-xl border border-gray-100 flex flex-col gap-2">
                                      <div className="flex items-center justify-between gap-3">
                                        <p className="text-sm font-bold text-gray-800 flex-1">{task.title}</p>
                                      ...
                                      {task.remarks && (
                                        <div className="mt-1 pt-2 border-t border-gray-200/50">
                                          ...
                                          <p className="text-xs text-gray-600 italic leading-relaxed">"{task.remarks}"</p>
                                        </div>
                                      )}
                                    </div>
                                  ))
                                  ...
                                  <div className="col-span-2 py-8 text-center bg-gray-50/50 rounded-xl border border-dashed border-gray-200">
                                    <p className="text-xs text-gray-400 font-medium italic">Legacy log: {log.reportTasksDone || 'No detailed tasks recorded'}</p>
                                  </div>
                                )}
                              </div>

                              <div className="pt-4 grid grid-cols-1 sm:grid-cols-2 gap-6 border-t border-gray-100">
                                <DetailItem label="Planned Client" value={log.plannedClient} icon={<MapPin size={12}/>} />
                                <DetailItem label="Department" value={log.plannedDept} />
                              </div>
                            </div>
                          </div>
                        </div>
        ...
        {totalPages > 1 && (
          <div className="p-5 bg-gray-50/50 border-t border-gray-100 flex items-center justify-between">
            <p className="text-sm text-gray-500">
              Showing <span className="font-semibold text-gray-900">{indexOfFirstItem + 1}</span> to <span className="font-semibold text-gray-900">{Math.min(indexOfLastItem, filteredLogs.length)}</span> of <span className="font-semibold text-gray-900">{filteredLogs.length}</span> employees
            </p>
...
const DetailItem = ({ label, value, icon, isMultiline }: any) => (
...
const AttendanceBadge = ({ status }: { status: string }) => {
  ...
  return (
    <span className={`px-3 py-1 rounded-full text-[10px] font-bold border ${currentStyle} uppercase tracking-tight`}>
      {status}
    </span>
  );
};
