      ...
      >
        ...
        >
          <div className="flex items-center gap-2.5 overflow-hidden">
            <div className={`flex items-center justify-center shrink-0 ${isCollapsed ? 'w-10 h-10' : 'w-8 h-8'}`}>
              <img 
                src="/logo-icon.png" 
                alt="Logo Icon" 
        ...
        <nav className="flex-1 overflow-y-auto py-6 px-3 space-y-1">
          ...
          <NavItem to="/employees" icon={<Users size={20} />} label="Employees" collapsed={isCollapsed} />
          <NavItem to="/clients" icon={<Briefcase size={20} />} label="Clients" collapsed={isCollapsed} />
          <NavItem to="/clauses" icon={<LayoutList size={20} />} label="Clauses" collapsed={isCollapsed} />
          <NavItem to="/schedule" icon={<Calendar size={20} />} label="Schedule" collapsed={isCollapsed} />
          <NavItem to="/attendance" icon={<CalendarClock size={20} />} label="Attendance Logs" collapsed={isCollapsed} />
        ...
        <div className="p-3 border-t border-gray-100 bg-gray-50/50">
          {!isCollapsed ? (
            <div className="animate-in fade-in duration-300">
              <div className="flex items-center gap-3 mb-4 px-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-gray-900 truncate" title={user?.fullname || 'User'}>
                    {user?.fullname || 'User'}
                  </p>
                  <p className="text-[11px] text-gray-500 truncate" title={user?.username ? `@${user.username}` : ''}>
            ...
            <div className="flex flex-col gap-4 items-center py-2">
              <div className="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 font-bold border border-indigo-200" title={user?.fullname}>
                {userInitials}
              </div>
              <button
                onClick={handleLogout}
      ...
      < div className="flex-1 flex flex-col min-w-0 overflow-hidden" >
        ...
        < header className="h-16 bg-white border-b border-gray-200 flex items-center justify-between px-6 lg:hidden shrink-0" >
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-sm text-gray-900">The Consulting Gurus</span>
          </div>
          <button
...
const NavItem = ({ to, icon, label, collapsed }: any) => (
  ...
  >
    <div className="flex items-center gap-3">
      ...
      <span className={`whitespace-nowrap transition-all duration-300 ${collapsed ? 'w-0 opacity-0 hidden' : 'w-auto opacity-100 block text-sm'
        }`}
      >
        {label}
      </span>
    ...
    {!collapsed && (
      <ChevronRight size={14} className="opacity-0 group-[.active]:opacity-100 transition-opacity text-indigo-300" />
    )}
  </NavLink>
);
