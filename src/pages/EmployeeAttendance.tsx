...
import {
  ArrowLeft, Calendar, Clock, MapPin, AlertCircle, CheckCircle, ArrowRight, ChevronLeft, ChevronRight
} from 'lucide-react';
import { format, eachDayOfInterval, startOfMonth, endOfMonth, isWeekend, getMonth, getYear } from 'date-fns';
...
const EmployeeAttendance = () => {
  ...
  const handleExportExcel = () => {
    ...
    XLSX.writeFile(wb, `Attendance_${employee.fullname.replace(/\s+/g, '_')}_${format(currentMonth, 'MMM_yyyy')}.xlsx`);
  };
  ...
  if (!data) return <div className="p-8 text-center">Employee not found or error loading data.</div>;

  const { employee, logs } = data;

  const today = new Date();
  const isCurrentMonth = getMonth(currentMonth) === getMonth(today) && getYear(currentMonth) === getYear(today);
  ...
  const lateCount = logs.filter((l: any) => l.isLate).length;

  // Calculate average hours (only for days with valid totalHrs)
  const validHoursLogs = logs.filter((l: any) => l.totalHrs && !isNaN(parseFloat(l.totalHrs)));
  const totalHours = validHoursLogs.reduce((sum: number, l: any) => sum + parseFloat(l.totalHrs), 0);
  const avgHours = validHoursLogs.length > 0 ? (totalHours / validHoursLogs.length).toFixed(2) : "0";
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-500 font-medium">{employee.role} • <span className="text-indigo-600">ID: #{employeeId}</span></p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden p-1">
              ...
              <button 
                ...
                disabled={format(currentMonth, 'MM-yyyy') === format(new Date(), 'MM-yyyy')}
            ...
            >
              <ArrowRight size={18} className="rotate-[-90deg] translate-y-[1px]" />
              Export Excel
            </button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
        <SummaryCard icon={<CheckCircle />} label="Days Present" value={presentCount} color="text-emerald-600 bg-emerald-50" />
        <SummaryCard icon={<Clock />} label="Late Arrivals" value={lateCount} color="text-amber-600 bg-amber-50" />
        {/* Note: Absent count is tricky without checking every past weekday, simplifying for now */}
        <SummaryCard icon={<AlertCircle />} label="Days Logged" value={logs.length} color="text-blue-600 bg-blue-50" />
        <SummaryCard icon={<Clock />} label="Avg. Daily Hrs" value={`${avgHours}h`} color="text-indigo-600 bg-indigo-50" />
      </div>

      {/* 3. DETAILED LOGS TABLE */}
      <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
        <div className="p-5 border-b border-gray-200 bg-gray-50/30">
          <h3 className="font-bold text-gray-800">Monthly Timecard</h3>
        </div>
          ...
          <table className="w-full text-left border-collapse min-w-[900px]">
            ...
            <tbody className="divide-y divide-gray-100">
              {daysInMonth.map((date) => {
                ...
                const status = log ? (log.status) : (isWeekendDay ? 'Weekend' : 'Absent');
                const location = log ? log.location : '--';
                const actualHrs = log ? parseFloat(log.totalHrs || 0) : 0;

                return (
                  <tr key={date.toString()} className="hover:bg-gray-50/50 transition-colors">
                    <td className="p-4 font-bold text-gray-900 text-sm">
                      {format(date, 'dd MMM, EEE')}
                    </td>
                    <td className="p-4">
                      <AttendanceBadge status={status} isLate={log?.isLate} />
                    </td>
                    <td className="p-4">
                      <div className="flex items-center gap-2 text-gray-600 text-sm font-medium">
                        {checkIn !== '--' && <Clock size={14} className="text-indigo-400" />}
                        {checkIn}
                      </div>
                    </td>
                    <td className="p-4">
                      <div className="flex items-center gap-2 text-gray-600 text-sm font-medium">
                        {checkOut !== '--' && <ArrowRight size={14} className="text-orange-400" />}
                        {checkOut}
                      </div>
                    </td>
                    <td className="p-4">
                      {log ? (
                        <div className="flex flex-col">
                          <span className={`text-sm font-bold ${actualHrs < REQUIRED_HOURS ? 'text-amber-600' : 'text-gray-900'}`}>
                            {actualHrs}h / {REQUIRED_HOURS}h
                          </span>
                        </div>
                      ) : (
                    ...
                    <td className="p-4">
                      <span className="text-xs font-bold text-gray-700">{log?.plannedClient || '--'}</span>
                    </td>
                    <td className="p-4">
                      {location !== '--' && (
...
const AttendanceBadge = ({ status, isLate }: { status: string, isLate?: boolean }) => {
  const styles: any = {
  ...
  return (
    <div className="flex gap-1">
      <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold border ${styles[status] || styles.Absent} uppercase tracking-tighter`}>
        {status}
      </span>
      {isLate && (
        <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold border ${styles.Late} uppercase tracking-tighter`}>
          Late
        </span>
      )}
    </div>
