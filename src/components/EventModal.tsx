...
const EventModal: React.FC<EventModalProps> = ({ isOpen, onClose, onSave, onDelete, initialData }) => {
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        start: initialData.start ? format(new Date(initialData.start), "yyyy-MM-dd'T'HH:mm") : '',
        end: initialData.end ? format(new Date(initialData.end), "yyyy-MM-dd'T'HH:mm") : '',
        type: initialData.extendedProps?.type || initialData.type || 'Meeting',
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-gray-400 uppercase tracking-widest px-1">Event Title</label>
            <input
              type="text"
              required
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-gray-400 uppercase tracking-widest px-1">Event Type</label>
