...
const Employees = () => {
	...
	const handleDelete = async (id: number) => {
		if (
			window.confirm(
				"Are you sure you want to delete this employee? This will also remove their logs.",
			)
		) {
			try {
			...
			<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
				<div>
					...
					<p className="text-gray-500 mt-1 text-sm font-medium">
						Manage your employees, roles, and work hours.
					</p>
				</div>
				<button
			...
			<div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden flex flex-col">
				...
				{!loading && users.length > 0 && (
					<div className="p-4 border-t border-gray-200 bg-gray-50/50 flex items-center justify-between text-xs font-bold text-gray-500">
						<span>
							SHOWING {users.length} OF {totalResults} EMPLOYEES
						</span>
					...
					<div className="overflow-y-auto p-6 custom-scrollbar">
						<form onSubmit={handleSubmit} className="space-y-4">
							...
							<div className="pt-4 flex gap-3 justify-end">
								...
								<button
									type="submit"
									className="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl text-sm shadow-lg shadow-indigo-100 transition-all">
									{modalMode === "ADD"
										? "Create Member"
										: "Save Changes"}
								</button>
