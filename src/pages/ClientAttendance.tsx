...
import {
  ArrowLeft, Calendar, Briefcase, FileText, ChevronLeft, ChevronRight, ArrowRight
} from 'lucide-react';
...
const ClientAttendance = () => {
  ...
  const handleExportExcel = async (mode: 'MONTH' | 'OVERALL') => {
    ...
    try {
      ...
      if (mode === 'MONTH') {
        ...
        filename = `Report_${data.client.name.replace(/\s+/g, '_')}_${format(currentMonth, 'MMM_yyyy')}.xlsx`;
      } else {
        // Fetch Overall
        const response = await getClientHistory(clientId!); // Omit month/year for 'Overall'
        logsToExport = response.data.logs;
        filename = `Overall_Report_${data.client.name.replace(/\s+/g, '_')}_${format(new Date(), 'dd_MMM_yyyy')}.xlsx`;
      }
  ...
  if (!data) return <div className="p-8 text-center">Client not found or error loading data.</div>;
    ...
    <div className="space-y-8 animate-in fade-in duration-500 relative">
        ...
        <div className="fixed inset-0 bg-black/40 z-[60] flex items-center justify-center p-6 backdrop-blur-sm animate-in fade-in duration-300">
           <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-sm w-full space-y-6">
              <div className="text-center">
                ...
                <p className="text-sm text-gray-500 mt-2">Which data would you like to export to Excel?</p>
              </div>
              ...
              <div className="space-y-3">
                ...
                >
                   <span className="font-bold text-gray-900">Selected Month</span>
                   <span className="text-xs text-gray-500">{format(currentMonth, 'MMMM yyyy')} records only</span>
                </button>
                ...
                <button 
                  ...
                  className="w-full p-4 border-2 border-gray-100 hover:border-indigo-600 hover:bg-indigo-50/50 rounded-2xl transition-all flex flex-col items-start gap-1 group"
                >
                   <span className="font-bold text-gray-900">Overall Report</span>
                   <span className="text-xs text-gray-500">Full platform history for this client</span>
                </button>
              </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-500 font-medium">Monthly Attendance Summary</p>
            </div>
          </div>

          <div className="flex items-center gap-3">
            <div className="flex items-center bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden p-1">
              <button onClick={handlePrevMonth} className="p-2 hover:bg-gray-100 rounded-lg text-gray-500 transition-colors">
                <ChevronLeft size={18} />
              </button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
            ...
            <p className="text-3xl font-black text-gray-900">{new Set(logs.map((l: any) => l.employeeName)).size}</p>
        </div>
        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Total Effort</p>
            <p className="text-3xl font-black text-indigo-600">
                {logs.reduce((sum: number, l: any) => sum + parseFloat(l.totalHrs), 0).toFixed(1)}h
            </p>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
        <div className="p-6 border-b border-gray-100">
          <h3 className="font-bold text-gray-800 tracking-tight">Deployment Logs</h3>
        </div>

        {logs.length === 0 ? (
          <div className="p-12 text-center text-gray-400">
            <FileText size={48} className="mx-auto mb-4 opacity-10" />
            <p>No activity recorded for this client in {format(currentMonth, 'MMMM yyyy')}.</p>
          </div>
        ) : (
                  ...
                  <tr key={log.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="p-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-gray-500 font-medium uppercase tracking-tight">{log.employeeRole}</p>
                        </div>
                      </div>
                    </td>
                    <td className="p-5">
                      <div className="flex items-center gap-4 text-xs font-bold">
                        <div className="flex flex-col">
                            <span className="text-gray-400 uppercase text-[9px] mb-0.5">IN</span>
                            <span className="text-gray-700">{log.clockIn}</span>
                        </div>
                        <ArrowRight size={12} className="text-gray-300" />
                        <div className="flex flex-col">
                            <span className="text-gray-400 uppercase text-[9px] mb-0.5">OUT</span>
                            <span className="text-gray-700">{log.clockOut}</span>
                        </div>
                      </div>
                    </td>
                    <td className="p-5">
                    ...
                    <td className="p-5 max-w-sm">
                      <p className="text-xs text-gray-600 leading-relaxed line-clamp-2">
                        {log.reportTasksDone !== '--' ? log.reportTasksDone : log.plannedTasks}
                      </p>
                    </td>
                  </tr>
                ))}
